...
有些应用需要大量的细粒度对象，这些对象彼此之间独立但却难以区分，彼此有许多相似之处，而不同之处在于其所处的外在环境。

享元对象可以同时在多个环境中分享和使用。享元对象在每一个环境里面都算作是独立的对象，与那些非共享的对象没有什么区别。享元对象不能对他们所处的环境作任何假设。

享元模式的关键概念是**内在**和**外在**的状态。内在状态包含了与享元对象所处环境独立的信息，外在则相反。当需要用到享元对象时，客户对象负责传递外在环境的状态给享元对象。

### 应用性

享元对象的效果的好坏，很大程度上去却与它是在哪和如何使用的。必须在下面的**所有**条件都成立时，才能使用享元模式。
- 使用大量的对象
- 储存大量对象的成本较高
- 大部分对象的状态能够是外在的，与环境相关的。
- 当外在状态被去除后，许多组的对象能够用少量的共享对象来表示。
- 应用程序不依赖对象的唯一性（identity）。享元对象会被共享，在对概念上有区别的两个对象的唯一性检查会返回他们相等的结果。

### 结构

![享元模式](/assets/pictures/studying/flyweight-pattern.png)

### 参与者

- Flyweight
  - 定义接口，通过此接口，享元对象能够接收和对外在状态做出相应行为。
- ConcreteFlyweight
  - 实现享元对象的接口，储存内在的状态，这些状态必须独立于外在环境，且这个对象必须能够被共享。
- UnsharedConcreteFlyweight
  - 不是所有享元对象的子类都需要被共享，对象可以被共享，但不是强制要求。这类对象通常都会在享元对象结构中储存具体享元对象。
- FlyweightFactory
  - 创建并管理享元对象
  - 保证享元对象被合理的共享。客户需要享元对象的时候，如果存在该享元对象，则直接返回，若不存在，则创建一个并返回。
- Client
  - 保存享元对象的引用
  - 计算或储存享元对象的外在状态
...
享元对象会引入一些转换、寻找和计算外在状态的成本，尤其在它原本是内在状态的情况下。
...
当对象使用大量的内在和外在状态，并且外在状态都可以被计算出来的时候，空间的储存成本节省最多。

享元模式通常与组合模式一起用，来代表一个有着共享叶子节点的树形结构。不过，叶子节点不能储存父对象的引用，相反的，父对象的引用会作为外在状态的一部分，传给叶子对象。

### 实现

需要考虑如下的问题
- *移除外在状态。*将外在状态从共享对象中剥离的容易程度，很大程度上决定了这个模式是否能够应用到此处。理想情况下，外在状态能够从一个独立的，有着更小储存需求的结构中计算出来。
- *管理共享对象。*客户不应该直接实例化享元对象。享元工厂提供创建和管理，以及查找的功能。另外，可以实现对象计数和垃圾收集来回收共享对象以节省空间，不过在共享对象确定且数量较少的情况下，不必做此步骤。
